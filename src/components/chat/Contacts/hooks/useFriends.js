...
export const useFriends = (currentUser) => {
  ...
  const fetchFriendRequests = useCallback(async () => {
    ...
    try {
      ...
      if (receivedRes.status === "fulfilled") {
        ...
        received = Array.isArray(d?.data) ? d.data : Array.isArray(d) ? d : [];
      }
      ...
      if (sentRes.status === "fulfilled") {
        ...
        sent = Array.isArray(d?.data) ? d.data : Array.isArray(d) ? d : [];
      }
  ...
  const getRelationStatus = useCallback(
    (userId) => {
      ...
      if (friendRequests.received.some((r) => String(r.id) === uid))
        return "RECEIVED";
      return "NONE";
    },
